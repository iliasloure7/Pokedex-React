...
function PokemonDetails({ data }: Props) {
  return (
    <div className='flex flex-col container max-w-sm mx-auto gap-6 uppercase'>
      <div className='flex bg-teal-800 p-3 rounded-lg items-center justify-between  text-white'>
        <p>{data.name}</p>
        <p>No. {data.id}</p>
      </div>
      <div className='flex bg-slate-200   rounded-lg shadow-md'>
      ...
      <div className='flex bg-gray-700 text-white rounded-lg shadow-md'>
        <div className='flex flex-col gap-2 p-4 basis-1/2 justify-center items-center border-r-2 border-gray-400'>
          <span>Hp</span>
          <span>Attack</span>
          <span>Defense</span>
